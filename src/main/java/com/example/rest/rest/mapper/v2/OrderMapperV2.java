...
@DecoratedWith(OrderMapperDelegate.class) //Указываем дополнительную логику для мапера
...
public interface OrderMapperV2 {
    ...
    List<OrderResponse> ordersListToResponseList(List<Order> orders);

    default OrderListResponse orderListToOrderListResponse(List<Order> orders) {
        OrderListResponse response = new OrderListResponse();
         response.setOrders(ordersListToResponseList(orders));
